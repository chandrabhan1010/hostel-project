<!--Navbar  -->
<nav class="top-nav " id="home">
    <div class="container-fluid">
        <div class="row justify-content-between">
            <div class="col-auto">
               <p>
                <i class='bx bxs-envelope' ></i>
                <span>ggbhostel@igntu.ac.in</span>
                </p>
                <p>
                <i class='bx bxs-phone-call' ></i>
                <span>(219) 555-0114</span>
                </p>
            </div>
            <div class="col-auto fs-5 socialicons">
                <div class="social-links">
                    <a href="#"><i class='bx bxl-facebook' ></i></a>
                    <a href="#"><i class='bx bxl-twitter' ></i></a>
                    <a href="#"><i class='bx bxl-google' ></i></a>
                    <a href="#"><i class='bx bxl-youtube' ></i></a> 
                </div>
            </div>
        </div>
    </div>
 </nav>    
 <nav class="navbar navbar-expand-lg navbar-light bg-white px-lg-3 py-ly-2 shadow-sm sticky-top">
        <div class="container-fluid">
           <a href="/hostelproject/index.php"> <img src="../images/hostellogo.png" class="logo"></a>
            <a class="navbar-brand me-4 fw-bold fs-3 h-font" href="index.php">GGBH HOSTEL<span>.</span></a>
            <button class="navbar-toggler shadow-none" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
            <span class="navbar-toggler-icon"></span>
            </button>

            <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                <li class="nav-item">
                <a class="nav-link" href="/hostelproject/index.php">Home</a>
                </li>
                <li class="nav-item">
                <a class="nav-link" href="/hostelproject/about_us.php">About us</a>
                </li>
                <li class="nav-item">
                <a class="nav-link" href="/hostelproject/admission.php">Admission</a>
                </li>
                <li class="nav-item">
                <a class="nav-link" href="/hostelproject/rules.php">Rules</a>
                </li>
                <li class="nav-item">
                <a class="nav-link" href="/hostelproject/infrastructure.php">Infrastructure</a>
                </li>
                <li class="nav-item">
                <div class="btn-group" role="group">
                    <a  id="btnGroupDrop1"  type="button" class="nav-link dropdown-toggle"data-bs-toggle="dropdown">Portal</a>
                  
                    <ul class="dropdown-menu" >
                            <li><a class="dropdown-item" href="/hostelproject/hostelfee.php">Submit Hostel Fee </a></li>
                            <li><a class="dropdown-item" href="/hostelproject/fee_receipt.php">Download Hostel Fee Recicpt</a></li>
                            <li><a class="dropdown-item" href="/hostelproject/hostel_receipt.php">Download Application Recicpt</a></li>
                    </ul>
                </div>
                </li>
                <li class="nav-item">
                <div class="btn-group" role="group">
                    <a  id="btnGroupDrop1"  type="button" class="nav-link dropdown-toggle"data-bs-toggle="dropdown">Registration form</a>
                    <ul class="dropdown-menu" >
                        <li><a class="dropdown-item " href="/hostelproject/ug_register.php">UG Registration </a></li>
                        <li><a class="dropdown-item " href="/hostelproject/pg_register.php">PG registration</a></li>
                    </ul>
                </div>
                </li>
                <li class="nav-item">
                <a class="nav-link" href="/hostelproject/notice.php">Notice</a>
                </li>
                <li class="nav-item">
                <a class="nav-link" href="/hostelproject/contact_us.php">Contact us</a>
                </li>
            </ul>
            <div class="d-flex">

            <button type="button" class="btn btn-brand shadow-none me-lg-3 me-2 "   data-bs-toggle="modal" data-bs-target="#usermodal">
            <img src="../images/usericon.png" class="fs-4 me-2 " style="height: 1rem; width:1rem;">User Login
           </button>

           <button type="button" class="btn btn-brand shadow-none me-lg-2 me-2 "   data-bs-toggle="modal" data-bs-target="#adminmodal">
           <img src="../images/igntulogo.png" class="fs-4 me-2 " style="height: 1rem; width:1rem;">Admin Login
           </button>
            </div>
        </div>
    </div>
</nav>

<!-- modal1 -->
  
<div class="modal fade" id="usermodal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
        <form >
            <div class="modal-header">
            <h5 class="modal-title d-flex align-items-center" >
                <img src="../images/usericon.png" class="fs-4 me-2 " style="height: 2rem; width:2rem;"> User Login</h5>
            <button type="reset" class="btn-close shadow-none" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">

            <div class="mb-3 text-start">
            <label class="form-label text-dark">Email address</label>
            <input type="email" class="form-control shadow-none">
            </div>

            <div class="mb-3 text-start">
            <label class="form-label text-dark">Passowrd</label>
            <input type="email" class="form-control shadow-none">
            </div>

            <div class="d-flex align-items-center justify-content-between md-4">
                <button type="submit" class="btn btn-brand">LOGIN</button>
                <a href="javascript:void(0)" class="text-secondary"> Forgot Passowrd?</a>
            </div>
            </div>
        </form>
    </div>
  </div>
</div>

<!-- Modal2 -->
<div class="modal fade" id="adminmodal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
        <form >
            <div class="modal-header">
            <h5 class="modal-title d-flex align-items-center" >
            <img src="../images/igntulogo.png" class="fs-4 me-2 " style="height: 2rem; width:2rem;">Admin Login</h5>
            <button type="reset" class="btn-close shadow-none" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">

            <div class="mb-3 text-start">
            <label class="form-label text-dark">Email address</label>
            <input type="email" class="form-control shadow-none">
            </div>

            <div class="mb-3 text-start">
            <label class="form-label text-dark">Passowrd</label>
            <input type="email" class="form-control shadow-none">
            </div>

            <div class="d-flex align-items-center justify-content-between md-4">
                <button type="submit" class="btn btn-brand">LOGIN</button>
                <a href="javascript:void(0)" class="text-secondary"> Forgot Passowrd?</a>
            </div>
            </div>
        </form>
    </div>
  </div>
</div>
