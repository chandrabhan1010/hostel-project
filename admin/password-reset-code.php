...
function  send_password_reset($get_name,$get_email,$token)
{
    ...
    try 
    {
        ...
        $mail->Body  = "<h2>Hello</h2>
                         ...
                         <br><br>
                         <a href='http://localhost:8080/HostelProject/admin/password-change.php?token=$token&email=$get_email'> Click Here</a>";
...
if(isset($_POST['password_reset_link']))
{
    ...
    $check_email = "SELECT * FROM admin_cred WHERE admin_email='$email' LIMIT 1";
    ...
    if(mysqli_num_rows($check_email_run))
    {
      ...
      $update_token ="UPDATE admin_cred SET verify_token='$token' WHERE admin_email='$email'LIMIT 1";
   ...
   $token = mysqli_real_escape_string($con,$_POST['password_token']);
    ...
    if(!empty($token))
    {
      if(!empty($email) && !empty($new_pass) && !empty($confirm_pass))
      {
       ...
       $check_token = "SELECT verify_token FROM admin_cred WHERE verify_token='$token' LIMIT 1";
       ...
       if(mysqli_num_rows($check_token_run)>0)
       {
         ...
         {
          $update_pass = "UPDATE admin_cred SET admin_pass='$new_pass' WHERE verify_token='$token'LIMIT 1";
          $update_pass_run = mysqli_query($con,$update_pass);
          if($update_pass_run)
          {
            $update_token = "UPDATE admin_cred SET verify_token='' WHERE admin_email='$email'LIMIT 1";
